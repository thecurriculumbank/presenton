...
export class PresentationGenerationApi {
  ...
  static async editSlide(slide_id: string, prompt: string) {
    try {
      const response = await fetch(`/api/v1/ppt/slide/edit`, {
        method: "POST",
        headers: getHeader(),
        body: JSON.stringify({
          id: slide_id,
  ...
  static async updatePresentationContent(body: any) {
    try {
      const response = await fetch(`/api/v1/ppt/presentation/update`, {
        method: "PATCH",
        headers: getHeader(),
  ...
  static async presentationPrepare(presentationData: any) {
    try {
      const response = await fetch(`/api/v1/ppt/presentation/prepare`, {
        method: "POST",
        headers: getHeader(),
  ...
  > => {
    try {
      const response = await fetch(`/api/v1/ppt/images/generated`, {
        method: "GET",
        headers: getHeader(),
      });
  ...
  // EXPORT PRESENTATION
  static async exportAsPPTX(presentationData: any, presentation_id: string) {
    try {
      const response = await fetch(`/api/v1/ppt/presentation/export/pptx`, {
        method: "POST",
        headers: getHeader(),
        body: JSON.stringify({ presentationData, presentation_id }),
        cache: "no-cache",
      });
      return await ApiResponseHandler.handleResponse(
        response,
