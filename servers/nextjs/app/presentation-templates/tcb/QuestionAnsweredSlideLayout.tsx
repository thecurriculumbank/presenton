...
export const Schema = z.object({
  ...
  actualAnswerIndex: z
    ...
    .max(3)
    .default(1)
    .describe("The index of the correct answer in the possibleAnswers array."),
});

export const layoutId = "QuestionAnsweredSlideLayout";
export const layoutName = "Question Answered Slide Layout";
export const layoutDescription =
  "Slide layout for answering the previous question";
...
const dynamicSlideLayout: React.FC<{
  ...
  const actualAnswerIndex = data.actualAnswerIndex;
          ...
          <div className="max-w-4xl w-full flex flex-col items-center gap-12 text-center">
            ...
            <div className="grid grid-cols-2 gap-6 w-full max-w-3xl">
              {possibleAnswers?.map((answer, idx) => {
                ...
                const isCorrect = idx === actualAnswerIndex;
                return (
                  <button
                    key={idx}
                    className={`relative flex items-center gap-6 text-left font-['Be_Vietnam_Pro',_sans-serif] font-bold text-lg transition-all duration-300 p-6 rounded-[60px] shadow-[0_2px_8px_rgba(27,28,21,0.04)] ${isCorrect ? "bg-white border-2 border-[#006780]" : "bg-[#f0f0f0] border-2 border-transparent"}`}
                  >
                    {/* Add a tick sticker to the top right corner if the answer is correct */}
                    {isCorrect && (
                      <div className="absolute -top-4 -right-4 w-8 h-8 bg-[#048000] rounded-full flex items-center justify-center text-white font-black text-2xl rotate-[5deg]">
                        ✓
                      </div>
                    )}
                    <div
